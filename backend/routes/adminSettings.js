...
router.put('/:key', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    await audit(req, 'SETTINGS_UPDATED', `Setting ${req.params.key} updated`, null, 'success', 'settings');
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});
...
router.post('/email-templates/test', auth, async (req, res) => {
  try {
    const { template_key, test_email, test_client_name, subject, sender_name, body } = req.body;
    if (!test_email) return res.status(400).json({ message: 'Test email address is required' });

    const testVars = {
      '{client_name}':       test_client_name || 'Test Client',
      '{rm_name}':           req.user?.name   || 'Test RM',
      '{optin_link}':        'https://clientiq.navia.in/optin/test-token',
      '{token_expiry_days}': '7',
      '{rm_phone}':          '9999999999',
      '{date}':              new Date().toLocaleDateString('en-IN'),
      '{churn_score}':       '75',
      '{expiry_date}':       new Date(Date.now() + 7*86400000).toLocaleDateString('en-IN'),
      '{digest_content}':    'This is a test digest.',
    };

    let testSubject = subject || `Test: ${template_key}`;
    let testBody    = body    || 'Test email body';
    Object.entries(testVars).forEach(([k, v]) => {
      testSubject = testSubject.split(k).join(v);
      testBody    = testBody.split(k).join(v);
    });
    ...
    await transporter.sendMail({
      from:    `"${sender_name || 'Navia ClientIQ'}" <alert@navia.co.in>`,
      to:      test_email,
      subject: `[TEST] ${testSubject}`,
      html:    wrapHtml(bodyToHtml(testBody)),
      text:    testBody,
    });
...
router.get('/audit-log', auth, async (req, res) => {
  try {
    ...
    const count = await pool.query(
      `SELECT COUNT(*) FROM audit_log al WHERE ${WHERE}`,
      params
    );
