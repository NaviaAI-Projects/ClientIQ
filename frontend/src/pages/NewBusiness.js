...
const NewBusiness = () => {
  ...
  const acctChartData = newAcctData.map(r => {
    // Try exact match first, then partial match (handles format differences)
    const ledger = ledgerData.find(l => l.month === r.month)
      || ledgerData.find(l => l.month?.slice(0,3) === r.month?.slice(0,3)) 
      || {};
    return {
      month:       r.month,
          ...
          {noData ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={clientsData} margin={{ top: 8, right: 4, bottom: 8, left: 4 }}>
                ...
                <Bar dataKey="eq_cash"     name="Eq Cash"      stackId="s" fill="#3B82F6" />
                <Bar dataKey="eq_options"  name="Eq Options"   stackId="s" fill="#10B981" />
                <Bar dataKey="comm_futures"name="Comm Futures"  stackId="s" fill="#F59E0B" radius={[3,3,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {acctChartData.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={acctChartData} margin={{ top: 8, right: 4, bottom: 8, left: 4 }}>
                ...
                <Bar dataKey="opened" name="New accounts opened" fill="#3B82F6" radius={[3,3,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
      ...
      <div className="panel">
        ...
        {acctChartData.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={acctChartData} margin={{ top: 8, right: 8, bottom: 30, left: 8 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.06)" />
              <XAxis dataKey="month" tick={{ fontSize: 10 }} angle={-30} textAnchor="end" height={60} />
              <YAxis tick={{ fontSize: 10 }} tickFormatter={FMT} />
              <Tooltip formatter={v => [FMT(v)]} />
              <Bar dataKey="new_balance" name="New client ledger balance (₹)" fill="#F59E0B" radius={[3,3,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
