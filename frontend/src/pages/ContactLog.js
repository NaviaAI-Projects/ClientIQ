...
const typeColor = { call: '#3B82F6', whatsapp: '#25D366', email: '#e67e22', meeting: '#8e44ad', note: '#888' };
const outcomeColor = {
  interested:     { bg: '#eaf3de', color: '#10B981' },
  not_interested: { bg: '#fcebeb', color: '#EF4444' },
  callback:       { bg: '#faeeda', color: '#F59E0B' },
  converted:      { bg: '#d4edda', color: '#1e7e34' },
  no_answer:      { bg: '#f5f5f5', color: '#888' },
  sent:           { bg: '#e8f3ff', color: '#3B82F6' },
  initiated:      { bg: '#e8f3ff', color: '#3B82F6' },
};
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: '16px', alignItems: 'start' }}>
        ...
        <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #eee', overflow: 'hidden', position: 'sticky', top: '20px' }}>
          <div style={{ background: '#1B3F7A', padding: '14px 18px' }}>
          ...
          <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <button onClick={saveInteraction} disabled={saving}
              style={{ padding: '10px', background: saving ? '#94a3b8' : '#1B3F7A', color: 'white', border: 'none', borderRadius: '8px', cursor: saving ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: '600' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '12px', flexWrap: 'wrap' }}>
            {['all', 'call', 'whatsapp', 'email', 'meeting'].map(f => (
              <button key={f} onClick={() => setFilter(f)}
                style={{ padding: '6px 14px', borderRadius: '20px', border: '1px solid ' + (filter === f ? '#1B3F7A' : '#ddd'),
                  background: filter === f ? '#1B3F7A' : 'white', color: filter === f ? 'white' : '#555',
