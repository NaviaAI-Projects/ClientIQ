...
const DailyMIS = () => {
  ...
  const CustomDot = (props) => {
    ...
    return <circle cx={cx} cy={cy} r={payload.is_expiry ? 5 : 3}
      fill={payload.is_expiry ? '#EF4444' : '#3B82F6'} stroke="none" />;
  };
        ...
        {dailyIncome.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={dailyIncome} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
              ...
              <Bar dataKey="options_clearing" name="Options clearing"   stackId="s" fill="#3B82F6" />
              <Bar dataKey="equity_brokerage" name="Equity brokerage"   stackId="s" fill="#10B981" />
              <Bar dataKey="mtf_interest"     name="MTF interest"       stackId="s" fill="#F59E0B" />
              <Bar dataKey="float_income"     name="Float income (est.)" stackId="s" fill="#8B5CF6" />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
          ...
          {dailyVol.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={dailyVol} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Line type="monotone" dataKey="eq_options_cr"   name="Eq Options (₹Cr)"   stroke="#3B82F6" fill="rgba(59,130,246,0.12)" strokeWidth={2} dot={<CustomDot />} />
                <Line type="monotone" dataKey="comm_options_cr" name="Comm Options (₹Cr)" stroke="#10B981" strokeWidth={1.5} dot={{ r: 3 }} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {dailyVol.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={dailyVol} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Tooltip />
                <Legend wrapperStyle={{ fontSize: 10 }} iconSize={10} />
                <Bar dataKey="fo_clients"     name="F&O clients"    fill="rgba(59,130,246,0.35)" />
                <Bar dataKey="equity_clients" name="Equity clients"  fill="#10B981" />
                <Bar dataKey="mtf_clients"    name="MTF clients"     fill="#F59E0B" />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
