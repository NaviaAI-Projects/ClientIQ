...
const AssignedLeads = () => {
  ...
  const getScoreBadge = (score) => {
    if (score >= 70) return { bg: '#fcebeb', color: '#EF4444', label: 'High' };
    if (score >= 50) return { bg: '#faeeda', color: '#F59E0B', label: 'Medium' };
    return { bg: '#e8f3ff', color: '#3B82F6', label: 'Low' };
  };
    ...
    <div>
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #eee', overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: '1px solid #eee', fontSize: '14px', fontWeight: '600', display: 'flex', justifyContent: 'space-between' }}>
          <span>⭐ My Leads</span>
          <span style={{ background: '#e8f3ff', color: '#3B82F6', padding: '2px 10px', borderRadius: '10px', fontSize: '12px', fontWeight: '600' }}>
            {leads.length} leads
          </span>
        </div>
        {loading ? (
                    ...
                    <tr key={lead.id}>
                      ...
                      <td style={td}>
                        <button onClick={() => navigate('/client-360')}
                          style={{ padding: '4px 10px', background: '#1B3F7A', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '12px', marginRight: '6px' }}>
                          View 360
                        </button>
                        <button onClick={() => navigate('/contact-log')}
                          style={{ padding: '4px 10px', background: 'white', color: '#1B3F7A', border: '1px solid #223872', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' }}>
                          Log
                        </button>
                      </td>
                    </tr>
