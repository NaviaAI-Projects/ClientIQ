  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px', marginBottom: '16px' }}>
        {[
          { label: 'Mapped Clients',  value: stats.clients,      color: '#1B3F7A', icon: '👥', sub: 'Total mapped' },
          { label: 'Active Leads',    value: stats.leads,        color: '#3B82F6', icon: '🎯', sub: 'Assigned to you' },
          { label: 'Interactions',    value: stats.interactions, color: '#10B981', icon: '📞', sub: 'Last 30 days' },
        ].map((s, i) => (
        ...
        {revenue.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={240}>
            <ComposedChart data={revenue} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
              ...
              <Bar dataKey="turnover" name="Total Turnover" fill="#3B82F6" radius={[4,4,0,0]} />
              <Bar dataKey="options"  name="Options"        fill="rgba(59,130,246,0.35)" radius={[4,4,0,0]} />
              <Line type="monotone" dataKey="target" name="Monthly Target" stroke="#EF4444" strokeDasharray="5 4" dot={false} strokeWidth={2} />
            </ComposedChart>
          </ResponsiveContainer>
        )}
      </div>
        ...
        <ResponsiveContainer width="100%" height={180}>
          <AreaChart data={clientGrowth} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
            ...
            <Area type="monotone" dataKey="clients" name="Mapped Clients"
              stroke="#3B82F6" fill="rgba(59,130,246,0.12)" strokeWidth={2} dot={{ r: 4, fill: '#1B3F7A' }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <div style={{ flex: 1, height: '6px', background: '#eee', borderRadius: '3px', maxWidth: '80px' }}>
                      <div style={{ width: row.score + '%', height: '100%', borderRadius: '3px',
                        background: row.score >= 80 ? '#10B981' : row.score >= 50 ? '#F59E0B' : '#EF4444' }} />
                    </div>
