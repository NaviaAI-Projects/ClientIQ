...
const ConcentrationRisk = () => {
  ...
  const revenueModel = [
    { name: 'Eq Options clearing (40%)', value: 40, color: '#3B82F6' },
    { name: 'Equity brokerage (30%)',    value: 30, color: '#10B981' },
    { name: 'Float income (20%)',        value: 20, color: '#8B5CF6' },
    { name: 'MTF interest (10%)',        value: 10, color: '#F59E0B' },
  ];

  // Float concentration — prototype exact data pattern
  const floatConc = [
    { label: 'Top 10',  pct: 18, fill: '#EF4444' },
    { label: 'Top 25',  pct: 28, fill: '#F59E0B' },
    { label: 'Top 50',  pct: 42, fill: '#F59E0B' },
    { label: 'Top 100', pct: 57, fill: '#3B82F6' },
    { label: 'Top 200', pct: 70, fill: '#10B981' },
    { label: 'Rest',    pct: 100, fill: '#94A3B8' },
  ];
              ...
              <BarChart data={concentration} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Bar dataKey="pct" name="% of Revenue" radius={[3,3,0,0]}>
                  {concentration.map((c, i) => (
                    <Cell key={i} fill={['#EF4444','#F59E0B','#F59E0B','#3B82F6','#3B82F6','#10B981','#94A3B8'][i] || '#3B82F6'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={trendFull} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Line type="monotone" dataKey="top10" name="Top 10 clients % of revenue"
                  stroke="#EF4444" fill="rgba(239,68,68,0.12)" strokeWidth={2} dot={{ r: 4 }} />
                <Line type="monotone" dataKey="top50" name="Top 50 clients % of revenue"
                  stroke="#3B82F6" fill="rgba(24,95,165,0.06)" strokeWidth={2} dot={{ r: 4 }} />
                <Line type="monotone" dataKey="target" name="Target — top 50 below 35%"
                  stroke="#10B981" strokeDasharray="6 4" dot={false} strokeWidth={1.5} />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
