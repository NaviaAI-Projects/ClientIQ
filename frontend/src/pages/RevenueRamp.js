...
const RevenueRamp = () => {
  ...
  const SPLIT_COLORS = {
    'Options activated': '#3B82F6',
    'Equity only':       '#10B981',
    'Never traded options': '#94A3B8'
  };
              ...
              <BarChart data={rampData} margin={{ top: 8, right: 4, bottom: 8, left: 4 }}>
                ...
                <Bar dataKey="options_pct" name="% with options activity" radius={[3,3,0,0]}>
                  {rampData.map((_, i) => (
                    <Cell
                      key={i}
                      fill={['#94A3B8','rgba(59,130,246,0.35)','#10B981','#3B82F6','#10B981','#3B82F6','#10B981','#10B981'][i] || '#3B82F6'}
                    />
                  ))}
                </Bar>
              </BarChart>
              ...
              <BarChart data={splitData} margin={{ top: 8, right: 4, bottom: 8, left: 4 }}>
                ...
                <Bar dataKey="avg_brokerage" name="Avg brokerage (₹/client)" radius={[3,3,0,0]}>
                  {splitData.map((entry, i) => (
                    <Cell key={i} fill={SPLIT_COLORS[entry.segment] || '#3B82F6'} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
