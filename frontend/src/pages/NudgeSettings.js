...
const NUDGE_TYPES = [
  { trigger:"Strike type performance",     example:'"Your last 14 Far OTM NIFTY CE trades: 38% win rate, avg loss ₹2,100. Your ATM trades: 64% win rate."', min:10, enabled:true  },
  { trigger:"Consecutive loss today",      example:'"You've had 3 losing trades today. Historically your win rate on 4th+ trades after losses is 41%."',      min:15, enabled:true  },
  { trigger:"Overtrading signal",          example:'"You've placed 9 trades today — days with 8+ trades show lower P&L outcomes in your trading history."',   min:20, enabled:true  },
  ...
  { trigger:"Oversized position",          example:'"This lot size is 2× your usual position. Your larger-than-average trades have a 44% win rate vs 61%."',    min:12, enabled:true  },
  { trigger:"Instrument underperformance", example:'"Your last 20 FINNIFTY CE trades: 40% win rate, net P&L –₹14,800. Your NIFTY CE trades: 63% win rate."',  min:10, enabled:true  },
];
...
const NudgeSettings = () => {
  ...
  return (
    <div>
      ...
      <div className="alert a-i">ℹ️ <strong>What nudges are:</strong> When a client is about to place an order, the system checks their 90-day trade history and surfaces a personalised data observation on the order screen. These are statistical summaries of the client's own data, not investment advice. Client opt-in consent is required.</div>
      ...
      <div className="panel">
        <div className="ptitle">⚙️ Nudge API &amp; OMS integration</div>
        <div className="fgrid">
          <div className="fgrp"><label>Nightly profile compute time</label><input type="time" defaultValue="02:00" /></div>
          <div className="fgrp"><label>Nudge API endpoint (for OMS to call)</label><input readOnly defaultValue="https://clientiq.navia.in/api/nudge" style={{background:"var(--bg2)",fontFamily:"monospace"}} /></div>
        ...
        <div className="fgrp" style={{marginBottom:"12px"}}>
          <label>Client opt-in consent text (shown at first nudge)</label>
          <textarea style={{minHeight:"60px"}} defaultValue="Navia may show you data-based observations from your own trading history during order placement. These are statistical summaries of your past trades, not investment advice. You can turn this off in Settings at any time." />
        </div>
        <div className="brow">
