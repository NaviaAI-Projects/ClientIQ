    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      {summary && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px', marginBottom: '16px' }}>
          {[
            { label: 'Total Turnover',       value: fmt(summary.total_turnover),   color: '#1B3F7A' },
            { label: 'Options Turnover',      value: fmt(summary.options_turnover), color: '#3B82F6' },
            { label: 'Eq Cash Turnover',      value: fmt(summary.eq_cash),          color: '#10B981' },
            { label: 'Brokerage Earned',      value: fmt(summary.brokerage),        color: '#F59E0B' },
          ].map((s, i) => (
        ...
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
              ...
              <Bar dataKey="eq_fo"      name="Eq F&O"     stackId="s" fill="#3B82F6" />
              <Bar dataKey="options"    name="Options"    stackId="s" fill="#3B82F6" />
              <Bar dataKey="commodity"  name="Commodity"  stackId="s" fill="#059669" radius={[4,4,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
        ...
        {data.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={data} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
              ...
              <Line type="monotone" dataKey="brokerage" name="Brokerage" stroke="#F59E0B" strokeWidth={2} dot={{ r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        )}
      </div>
            ...
            {(summary?.clients || []).length === 0 ? (
              ...
              (summary?.clients || []).map((c, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f5f5f5' }}>
                  ...
                  <td style={{ padding: '12px 14px', color: '#F59E0B', fontWeight: '600' }}>{fmt(c.brokerage)}</td>
                </tr>
              ))
            )}
          </tbody>
