...
const RMPipeline = () => {
  ...
  const btn = (s) => ({ padding: '8px 20px', background: saving === s ? '#94a3b8' : '#1B3F7A', color: 'white', border: 'none', borderRadius: '6px', cursor: saving === s ? 'not-allowed' : 'pointer', fontSize: '13px', fontWeight: '600' });

  const showMsg = (key) => msg === key + '_success'
    ? <span style={{ color: '#10B981', fontSize: '12px', marginLeft: '10px' }}>✓ Saved</span>
    : msg === key + '_error'
    ? <span style={{ color: '#EF4444', fontSize: '12px', marginLeft: '10px' }}>✗ Failed</span>
    : null;
                  ...
                  <tr key={i}>
                    ...
                    <td style={td}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontWeight: '600', color: available > 20 ? '#10B981' : '#EF4444' }}>{available}</span>
                        <div style={{ flex: 1, height: '6px', background: '#eee', borderRadius: '3px', maxWidth: '80px' }}>
                          <div style={{ width: pct + '%', height: '100%', background: pct > 80 ? '#e74c3c' : '#1B3F7A', borderRadius: '3px' }} />
                        </div>
                    ...
                    <td style={td}>
                      <span style={{ padding: '2px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '600',
                        ...
                        color: rm.status === 'active' ? '#10B981' : '#EF4444' }}>
                        {rm.status || 'active'}
                      </span>
                    </td>
                  </tr>
