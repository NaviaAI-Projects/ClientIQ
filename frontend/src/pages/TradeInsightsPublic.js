...
const TradeInsightsPublic = () => {
  ...
  if (token) {
    ...
    // Validate token by fetching client info from public endpoint
    api.post('/trade-insights/public', { ucc, token })
      .then(res => {
        setClient(res.data);
        setLoading(false);
      ...
      .catch(err => {
        setError(
          ...
          'Access denied. Please reopen from your trading platform.'
        );
        setLoading(false);
      });
  }, []); // eslint-disable-line
    ...
    <div style={{ background: '#F1F4F9', minHeight: '100vh', fontFamily: "'Manrope', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span style={{
            fontSize:      '16px', fontWeight: '800', color: '#223872',
          ...
          <span style={{
            ...
            background: '#EDEFF6', color: '#223872', borderRadius: '20px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{
            fontSize: '11px', color: '#8A96AC', fontFamily: 'monospace'
          }}>
            {ucc}
          </div>
          <div style={{
            fontSize: '13px', fontWeight: '600', color: '#223872'
          }}>
            {client?.client_name || ''}
          </div>
          <div style={{
