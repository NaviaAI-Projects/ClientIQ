          ...
          {noData ? (
            ...
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={rmData} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Bar dataKey="pre_rev"  name="Pre-mapping revenue (₹)"  fill="#94A3B8" />
                <Bar dataKey="post_rev" name="Post-mapping revenue (₹)" fill="#3B82F6" radius={[3,3,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
          ...
          {noData ? (
            ...
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={rmData} margin={{ top: 8, right: 8, bottom: 8, left: 8 }}>
                ...
                <Legend wrapperStyle={{ fontSize: 11 }} />
                <Bar dataKey="pre_vol"  name="Pre-mapping options TO (₹Cr)"  fill="#94A3B8" />
                <Bar dataKey="post_vol" name="Post-mapping options TO (₹Cr)" fill="#10B981" radius={[3,3,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
            ...
            <tbody>
              {rmData.map((rm, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #F1F4F9' }}>
                  ...
                  <td style={{ padding: '10px', color: '#3B82F6', fontWeight: '600' }}>
                    {FMT(rm.post_rev)}
                  </td>
