...
const ClientInsightEmail = () => {
  ...
  const btnP  = { padding: '7px 14px', background: '#1B3F7A', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' };
  const btnS  = { padding: '7px 14px', background: 'white', color: '#1B3F7A', border: '0.5px solid #223872', borderRadius: '5px', cursor: 'pointer', fontSize: '12px' };
      ...
      <div style={panel}>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={sendNow} disabled={sending}
            style={{ ...btnP, background: sending ? '#94a3b8' : '#10B981' }}>
            {sending ? '⏳ Sending...' : '🚀 Send Now to All Clients'}
          </button>
          {testMsg && <span style={{ fontSize: '12px', color: testMsg.includes('✅') ? '#10B981' : '#EF4444' }}>{testMsg}</span>}
        </div>
      </div>
        ...
        <div style={panel}>
          ...
          <div style={{ background: '#f5f4f0', borderRadius: '8px', padding: '16px 20px', fontSize: '12.5px', lineHeight: '1.8', border: '0.5px solid rgba(0,0,0,0.1)' }}>
            ...
            {config.include_float_nudge === 'Yes — if avg float > ₹2L' && preview.float && (
              <div style={{ background: '#e8f3ff', borderRadius: '6px', padding: '12px', margin: '12px 0', border: '0.5px solid rgba(24,95,165,0.2)', color: '#3B82F6' }}>
              ...
              {dispatchLog.map((d, i) => (
                <tr key={i} style={{ borderBottom: '0.5px solid rgba(0,0,0,0.05)' }}>
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    <span style={{ background: d.status === 'sent' ? '#eaf3de' : '#faeeda', color: d.status === 'sent' ? '#10B981' : '#F59E0B', padding: '2px 8px', borderRadius: '4px', fontSize: '11px', fontWeight: '600' }}>
                      {d.status}
                    </span>
                  </td>
                </tr>
